...
if [ $USERID -ne 0 ]
then
     echo -e " $R ERROR::: You are not running this script with Root User $N " &>>$LOG_FILE
     exit 1
else
     echo -e " $G You are running with Root User $N " &>>$LOG_FILE
fi
...
if [ $? -ne 0 ]
then
     echo -e " $R Python3 is not installed..... Going to install it $N " &>>$LOG_FILE
...
if [ $? -ne 0 ]
then
     echo -e " $R Nginx is not installed......Going to install it $N " &>>$LOG_FILE
